...
export const CreateCertificateView: React.FC = () => {
  ...
  const validate = () => {
    ...
    const invalidGrade = manualGrades.findIndex(value => value.trim() !== '' && (Number.isNaN(Number(value.replace(',','.'))) || Number(value.replace(',','.')) < 0 || Number(value.replace(',','.')) > 10));
    if (invalidGrade >= 0) return showError(`Nota inválida em ${course.syllabus?.[invalidGrade]?.discipline||`disciplina ${invalidGrade+1}`}. Informe um valor de 0 a 10.`), false;
    return true;
  };
  ...
  const preflightChecks=[
    {label:'Dados do condutor',ok:Boolean(studentName.trim()&&isValidCpf(studentDocument)&&registrationNumber.length===11&&cnhCategory),detail:'Nome, CPF, registro e categoria CNH'},
    {label:'Período e emissão',ok:Boolean(startDate&&endDate&&startDate<=endDate&&issueDate&&issueDate>=endDate&&location.trim()),detail:'Datas e local de emissão'},
    {label:'Curso e conteúdo',ok:Boolean(course&&course.workloadHours>0&&course.syllabus?.length),detail:'Carga horária e conteúdo programático'},
    {label:'Dados institucionais',ok:Boolean(institution.name&&institution.institutionCnpj&&institution.legalInstruction&&institution.contranResolution),detail:'Instituição, CNPJ e referências legais'},
    {label:'Assinatura do diretor',ok:institution.signatureMode==='govbr'||Boolean(institution.signatureMode==='image'&&institution.signatureImageUrl),detail:institution.signatureMode==='none'?'Nenhuma assinatura configurada':'Assinatura preparada'},
  ];
