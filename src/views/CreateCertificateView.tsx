...
export const CreateCertificateView: React.FC = () => {
  ...
  const [registrationNumber, setRegistrationNumber] = useState(initialStudent?.registrationNumber || '');
  ...
  useEffect(() => {
    ...
    setStudentName(student.fullName); setStudentEmail(student.email || ''); setStudentDocument(student.documentNumber || ''); setRegistrationNumber(student.registrationNumber || ''); setCnhCategory(student.cnhCategory || 'AD');
  ...
  const validate = () => {
    ...
    if (!studentDocument.trim()) return alert('Informe o CPF do aluno.'), false;
    if (!isValidCpf(studentDocument)) return alert('O CPF informado é inválido.'), false;
    if (!registrationNumber.trim()) return alert('Informe o Nº REGISTRO do condutor.'), false;
  ...
  return <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-6xl mx-auto w-full">
    ...
    {currentStep === 1 ? <form onSubmit={(e) => { e.preventDefault(); if (validate()) setCurrentStep(2); }} className="space-y-5">
      <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-3"><input value={studentName} onChange={(e) => setStudentName(e.target.value)} placeholder="Nome completo *" className="px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border" /><input value={studentEmail} onChange={(e) => setStudentEmail(e.target.value)} placeholder="E-mail" type="email" className="px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border" /><input value={studentDocument} onChange={(e) => setStudentDocument(e.target.value)} placeholder="CPF *" className="px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border" /><input value={registrationNumber} onChange={(e) => setRegistrationNumber(e.target.value)} placeholder="Nº REGISTRO *" className="px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border" /><input value={cnhCategory} onChange={(e) => setCnhCategory(e.target.value.toUpperCase())} placeholder="Categoria CNH (ex.: AD) *" className="px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border" /></div>
      </section>
