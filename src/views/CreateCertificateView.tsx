...
export const CreateCertificateView: React.FC = () => {
  ...
  const certificateSyllabus = useMemo(() => {
    const grades = selectedClass?.studentGrades?.[selectedStudentId] || [];
    return course?.syllabus?.map((item, index) => ({
      ...item,
      grade: grades[index]?.trim() || item.grade,
    }));
  }, [course, selectedClass, selectedStudentId]);
