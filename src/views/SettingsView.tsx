...
import { CheckCircle, ExternalLink, FileSignature, Moon, RotateCcw, Save, Settings, Sun, Trash2, Upload } from 'lucide-react';
...
export const SettingsView: React.FC = () => {
  ...
  const [signatureMode, setSignatureMode] = useState<'none'|'image'|'govbr'>(institution.signatureMode || (institution.signatureImageUrl ? 'image' : 'none'));
  const [signatureImageUrl, setSignatureImageUrl] = useState(institution.signatureImageUrl || '');
  const [signatureScale, setSignatureScale] = useState(institution.signatureScale || 100);
  const [signatureOffsetX, setSignatureOffsetX] = useState(institution.signatureOffsetX || 0);
  const [signatureOffsetY, setSignatureOffsetY] = useState(institution.signatureOffsetY || 0);
  const [signatureError, setSignatureError] = useState('');
  const [saved, setSaved] = useState(false);

  const selectSignature = (file?: File) => {
    setSignatureError('');
    if (!file) return;
    if (!['image/png','image/jpeg','image/webp'].includes(file.type)) { setSignatureError('Use uma imagem PNG, JPG ou WebP.'); return; }
    if (file.size > 2 * 1024 * 1024) { setSignatureError('A imagem deve ter no máximo 2 MB.'); return; }
    const reader = new FileReader();
    reader.onload = () => {
      const img = new Image();
      img.onload = () => {
        const ratio = Math.min(1, 800 / img.width, 300 / img.height);
        const canvas = document.createElement('canvas');
        canvas.width = Math.max(1, Math.round(img.width * ratio));
        canvas.height = Math.max(1, Math.round(img.height * ratio));
        const ctx = canvas.getContext('2d');
        if (!ctx) { setSignatureError('Não foi possível processar a imagem.'); return; }
        ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
        const pixels = ctx.getImageData(0, 0, canvas.width, canvas.height);
        for (let i=0;i<pixels.data.length;i+=4) {
          const light=(pixels.data[i]+pixels.data[i+1]+pixels.data[i+2])/3;
          if(light>242) pixels.data[i+3]=0;
          else if(light>210) pixels.data[i+3]=Math.round(255*(242-light)/32);
        }
        ctx.putImageData(pixels,0,0);
        setSignatureImageUrl(canvas.toDataURL('image/png'));
        setSignatureMode('image');
      };
      img.onerror = () => setSignatureError('Não foi possível processar a imagem.');
      img.src = String(reader.result);
    };
    reader.onerror = () => setSignatureError('Não foi possível ler a imagem.');
    reader.readAsDataURL(file);
  };

  const save = (event: React.FormEvent) => {
    event.preventDefault();
    updateInstitution({ name: name.trim(), institutionCnpj: institutionCnpj.trim(), city: city.trim(), state: state.trim().toUpperCase(), legalInstruction: legalInstruction.trim(), contranResolution: contranResolution.trim(), validityText: validityText.trim(), signatoryName: signatoryName.trim(), signatoryRole: signatoryRole.trim(), signatoryCpf: signatoryCpf.trim(), signatureMode, signatureImageUrl: signatureMode === 'image' ? signatureImageUrl : '', signatureScale, signatureOffsetX, signatureOffsetY, codeFormat: 'cvte' });
  ...
  return <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-4xl mx-auto w-full">
    ...
    <form onSubmit={save} className="space-y-5">
      ...
      <section className="bg-white dark:bg-slate-900 rounded-2xl border p-6 space-y-4"><div><h2 className="font-bold flex items-center gap-2"><FileSignature className="w-5 h-5 text-indigo-600"/>Assinatura do Diretor-Geral</h2><p className="text-xs text-slate-500">Configure os dados do responsável e escolha como o documento será assinado.</p></div><div className="grid sm:grid-cols-3 gap-3"><label className="text-xs font-semibold">Nome<input value={signatoryName} onChange={e=>setSignatoryName(e.target.value)} className="mt-1 w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border"/></label><label className="text-xs font-semibold">Função<input value={signatoryRole} onChange={e=>setSignatoryRole(e.target.value)} className="mt-1 w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border"/></label><label className="text-xs font-semibold">CPF<input value={signatoryCpf} onChange={e=>setSignatoryCpf(e.target.value)} className="mt-1 w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border"/></label></div><div className="grid sm:grid-cols-3 gap-3">{([['none','Sem assinatura visual','Exibe somente nome e função.'],['image','Imagem autorizada','Insere uma assinatura PNG/JPG no certificado.'],['govbr','Assinar pelo GOV.BR','Gera o PDF para assinatura digital posterior.']] as const).map(([id,title,text])=><button type="button" key={id} onClick={()=>setSignatureMode(id)} className={`text-left rounded-xl border p-4 ${signatureMode===id?'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/30':'border-slate-200 dark:border-slate-700'}`}><strong className="block text-sm">{title}</strong><span className="text-xs text-slate-500">{text}</span></button>)}</div>{signatureMode==='image'&&<div className="rounded-xl border border-dashed p-4"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white"><Upload className="w-4 h-4"/>Selecionar assinatura<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={e=>selectSignature(e.target.files?.[0])}/></label>{signatureError&&<p role="alert" className="mt-2 text-xs text-rose-600">{signatureError}</p>}{signatureImageUrl&&<div className="mt-4 flex items-center gap-4"><img src={signatureImageUrl} alt="Prévia da assinatura" className="h-20 max-w-[280px] object-contain bg-white rounded-lg border"/><button type="button" onClick={()=>setSignatureImageUrl('')} className="text-xs font-bold text-rose-600">Remover imagem</button></div>}{signatureImageUrl&&<div className="mt-4 grid sm:grid-cols-3 gap-3"><label className="text-xs font-semibold">Tamanho: {signatureScale}%<input type="range" min="70" max="150" value={signatureScale} onChange={e=>setSignatureScale(Number(e.target.value))} className="w-full"/></label><label className="text-xs font-semibold">Horizontal: {signatureOffsetX}px<input type="range" min="-50" max="50" value={signatureOffsetX} onChange={e=>setSignatureOffsetX(Number(e.target.value))} className="w-full"/></label><label className="text-xs font-semibold">Vertical: {signatureOffsetY}px<input type="range" min="-30" max="30" value={signatureOffsetY} onChange={e=>setSignatureOffsetY(Number(e.target.value))} className="w-full"/></label></div>}<p className="mt-3 text-xs text-amber-700">O fundo claro é removido automaticamente. Use somente uma assinatura fornecida e autorizada pelo próprio Diretor-Geral.</p></div>}{signatureMode==='govbr'&&<div className="rounded-xl bg-blue-50 border border-blue-200 p-4 text-sm text-blue-900"><p className="font-bold">Fluxo oficial GOV.BR</p><ol className="mt-2 list-decimal pl-5 text-xs space-y-1"><li>Salve esta configuração.</li><li>Gere o PDF completo do certificado.</li><li>Abra o assinador oficial e envie o PDF.</li><li>Baixe o documento assinado e valide no ITI.</li></ol><div className="mt-3 flex flex-wrap gap-2"><a href="https://assinador.iti.br/assinatura/index.xhtml" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-blue-700 px-3 py-2 text-xs font-bold text-white">Abrir assinador <ExternalLink className="w-3 h-3"/></a><a href="https://validar.iti.gov.br/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg border border-blue-300 px-3 py-2 text-xs font-bold">Validar assinatura <ExternalLink className="w-3 h-3"/></a></div><p className="mt-2 text-xs">O PDF só terá assinatura digital válida depois de concluído no portal oficial.</p></div>}</section>
