  ...
  const filteredCertificates = useMemo(() => {
    return certificates.filter((cert) => {
      const matchSearch =
        ...
        (cert.studentDocument && cert.studentDocument.includes(searchTerm));

      const matchCourse =
        selectedCourseFilter === 'all' || cert.courseId === selectedCourseFilter;

      const matchStatus =
        selectedStatusFilter === 'all' || cert.status === selectedStatusFilter;
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Consulte, filtre, visualize, duplique ou cancele certificados emitidos.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col md:flex-row items-center gap-3">
        ...
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Buscar por aluno, CPF, curso ou código (ex: CERT-2026)..."
            value={searchTerm}
        ...
        {filteredCertificates.length === 0 ? (
          <div className="p-12 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500">
              Tente redefinir os filtros ou criar um novo certificado para um aluno.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40 text-slate-500 font-bold uppercase tracking-wider">
                  <th className="px-5 py-3.5">Código / Autenticidade</th>
                  <th className="px-5 py-3.5">Aluno</th>
                  <th className="px-5 py-3.5">Curso & Carga Horária</th>
                  ...
                  >
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/60 px-2 py-1 rounded">
                          {cert.code}
                        </span>
                        <button
                          onClick={() => handleCopyCode(cert.code, cert.id)}
                      ...
                      {cert.uuid && (
                        <div className="text-[10px] font-mono text-slate-400 mt-1 truncate max-w-[140px]" title={cert.uuid}>
                          UUID: {cert.uuid.substring(0, 13)}...
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-5 py-4">
                      ...
                      <div className="text-[11px] text-slate-500">
                        {cert.workloadHours} horas • {cert.modality}
                      </div>
                    </td>

                    <td className="px-5 py-4 text-slate-600 dark:text-slate-400">
                      {cert.issueDate ? new Date(cert.issueDate + 'T00:00:00').toLocaleDateString('pt-BR') : '-'}
                    </td>
                    ...
                    <td className="px-5 py-4 text-right space-x-2">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900 text-indigo-700 dark:text-indigo-300 font-semibold transition-colors"
                      >
                        Ver / Baixar
                      </button>
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-medium transition-colors"
