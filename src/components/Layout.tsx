      ...
      <header className="md:hidden flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-40">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-[10px] block text-indigo-600 dark:text-indigo-400 font-semibold -mt-1">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 shrink-0 select-none justify-between h-screen sticky top-0">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <h1 className="font-extrabold text-lg tracking-tight text-slate-900 dark:text-white">
                    Certify<span className="text-indigo-600 dark:text-indigo-400">AI</span>
                  </h1>
                  <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 border border-indigo-200/50 dark:border-indigo-800/50">
                    SaaS
                  </span>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[140px]">
                  {institution.name || 'Tech Academy'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="p-3 border-t border-slate-100 dark:border-slate-800/80 space-y-3">
          ...
          <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200/70 dark:border-slate-800 text-xs">
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => switchUserRole(user?.role === 'admin' ? 'instructor' : 'admin')}
                className="w-full py-1 px-2 rounded text-[11px] font-medium bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 transition-colors"
                title="Alternar entre Administrador e Instrutor"
              >
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <button
              onClick={toggleTheme}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
              title={theme === 'dark' ? 'Mudar para Tema Claro (Principal)' : 'Mudar para Tema Escuro'}
              aria-label="Alternar tema"
            >
              {theme === 'dark' ? (
                <>
                  <Sun className="w-3.5 h-3.5 text-amber-400" />
                  <span className="text-[11px]">Tema Claro</span>
                </>
              ) : (
                <>
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex">
          <div className="w-72 bg-white dark:bg-slate-900 h-full p-4 flex flex-col justify-between shadow-2xl">
            <div>
              <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className="p-1.5 text-slate-500 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 text-xs">
              ...
              <button
                onClick={() => {
                  switchUserRole(user?.role === 'admin' ? 'instructor' : 'admin');
                  setMobileMenuOpen(false);
                }}
                className="w-full py-2 px-3 rounded-lg bg-slate-100 dark:bg-slate-800 font-medium text-slate-700 dark:text-slate-300"
              >
                Alternar para {user?.role === 'admin' ? 'Instrutor' : 'Admin'}
              </button>
            </div>
