...
export const CreateCertificateView: React.FC = () => {
  ...
  useEffect(() => { if (!selectedStudentId) return; const s=students.find(x=>x.id===selectedStudentId); if(!s)return; setStudentName(s.fullName);setStudentEmail(s.email||'');setStudentDocument(onlyDigits(s.documentNumber||''));setRegistrationNumber(onlyDigits(s.registrationNumber||''));setCnhCategory(s.cnhCategory||'AD'); }, [selectedStudentId, students]);
  useEffect(() => { localStorage.setItem(DRAFT_KEY, JSON.stringify({studentName,studentEmail,studentDocument,registrationNumber,cnhCategory,startDate,endDate,issueDate,location})); }, [studentName,studentEmail,studentDocument,registrationNumber,cnhCategory,startDate,endDate,issueDate,location]);

  useEffect(()=>{sessionStorage.removeItem('certifyai_prefill_class');if(!selectedClass)return;setStartDate(selectedClass.startDate);setEndDate(selectedClass.endDate);setValidityYears(selectedClass.validityYears||5);setSelectedStudentId('');setStudentName('');setStudentDocument('');setRegistrationNumber('');},[classId]);
  const validate = () => {
    setFormError('');
  ...
  return <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-6xl mx-auto w-full">
    <div><div className="inline-flex px-2.5 py-1 rounded-full bg-amber-100 text-amber-800 text-xs font-bold">Modelo Oficial • CVTE</div><h1 className="text-2xl sm:text-3xl font-extrabold mt-2">Emitir Certificado</h1><p className="text-sm text-slate-500">Preencha apenas os dados variáveis do condutor. O restante vem da configuração oficial.</p></div>
    <label className="block font-semibold">Turma<select className="block mt-2 w-full border rounded-xl p-3 bg-white dark:bg-slate-800" value={classId} onChange={e=>{setClassId(e.target.value);setCurrentStep(1);}}><option value="">Emissão avulsa</option>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
