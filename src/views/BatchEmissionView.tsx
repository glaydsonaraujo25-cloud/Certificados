...
import { AlertTriangle, CheckCircle2, Download, Eye, FileSpreadsheet, Layers, Loader2, Plus, RotateCcw, Trash2, Upload } from 'lucide-react';
...
export const BatchEmissionView: React.FC = () => {
  ...
  const validateRows = (inputRows: BatchRow[], targetCourse = course) => {
    ...
    return inputRows.map((row) => {
      ...
      else if (targetCourse && certificates.some((cert) => cert.status === 'active' && cert.courseId === targetCourse.id && normalizeDocument(cert.studentDocument || '') === document)) errorMsg = 'Já existe certificado ativo para este CPF neste curso.';
  ...
  useEffect(() => {
    setRows((prev) => validateRows(prev, course));
    setIssued([]);
  }, [selectedCourseId]);
  ...
  const exportIssuedReport = () => {
    ...
    const data = issued.map((cert) => ({
      codigo: cert.code,
      aluno: cert.studentName,
      cpf: cert.studentDocument || '',
      email: cert.studentEmail || '',
      curso: cert.courseName,
      carga_horaria: cert.workloadHours,
      data_emissao: cert.issueDate,
      status: cert.status === 'active' ? 'Ativo' : cert.status,
    }));
    const worksheet = XLSX.utils.json_to_sheet(data);
    worksheet['!cols'] = [{ wch: 22 }, { wch: 36 }, { wch: 18 }, { wch: 30 }, { wch: 36 }, { wch: 16 }, { wch: 16 }, { wch: 12 }];
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Certificados emitidos');
    XLSX.writeFile(workbook, `relatorio_certificados_${issueDate || new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const importExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!parsed.length) throw new Error('Planilha sem registros.');
  ...
  const processBatch = async () => {
    ...
    if (!issueDate) { alert('Informe a data de emissão dos certificados.'); return; }
      ...
      const results = rows.map((row) => {
        ...
        if (!student) {
          student = addStudent({
            ...
            notes: `Aluno importado por planilha Excel para emissão de certificado em ${issueDate}.`,
          });
        }
  ...
  return <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto w-full">
    <div><div className="flex items-center gap-2"><Layers className="w-6 h-6 text-indigo-600" /><h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white">Gerar por Planilha Excel</h1></div><p className="text-sm text-slate-500 mt-1">Importe uma planilha .xlsx ou .xls, revise os dados e gere certificados em lote.</p></div>

    <div className="grid lg:grid-cols-4 gap-4">
      <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-3"><div className="font-bold">Configuração da emissão</div><label className="block text-xs font-semibold">Curso<select value={selectedCourseId} onChange={(e) => setSelectedCourseId(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">{courses.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="block text-xs font-semibold">Data de emissão<input type="date" value={issueDate} onChange={(e) => setIssueDate(e.target.value)} className="mt-1 w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700" /></label>{course && <div className="text-xs text-slate-500">{course.workloadHours} horas • {course.modality} • término cadastrado em {new Date(course.endDate + 'T00:00:00').toLocaleDateString('pt-BR')}</div>}</div>
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {(rows.length > 0 || fileName) && <button onClick={clearBatch} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-50 dark:bg-rose-950 text-rose-700 dark:text-rose-300 text-sm font-bold"><RotateCcw className="w-4 h-4" />Limpar lote</button>}
      {fileName && <span className="flex items-center px-3 text-xs text-slate-500">Arquivo: {fileName}</span>}
    </div>

    <div className="rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-xs text-indigo-800 dark:border-indigo-900 dark:bg-indigo-950/40 dark:text-indigo-200">A primeira linha da planilha deve conter <strong>nome_aluno</strong>, <strong>email</strong> e <strong>cpf</strong>. A data de emissão e o curso são escolhidos acima e aplicados a todo o lote.</div>
    ...
    <div className="flex justify-end"><button onClick={processBatch} disabled={processing || !rows.length || invalidCount > 0 || !issueDate || !course} className="flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold disabled:opacity-50 disabled:cursor-not-allowed">{processing ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileSpreadsheet className="w-4 h-4" />}Gerar {validCount} certificado(s)</button></div>

    {issued.length > 0 && <section className="bg-white dark:bg-slate-900 rounded-2xl border border-emerald-200 dark:border-emerald-900 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold text-emerald-700">Geração concluída: {issued.length} certificado(s)</h2><p className="text-xs text-slate-500 mt-1">Todos foram registrados no gerenciador de certificados.</p></div><button onClick={exportIssuedReport} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 text-sm font-bold"><FileSpreadsheet className="w-4 h-4" />Baixar relatório Excel</button></div><div className="mt-3 space-y-2">{issued.map((cert) => <div key={cert.id} className="flex items-center justify-between rounded-xl bg-slate-50 dark:bg-slate-800 p-3"><div><div className="font-bold text-sm">{cert.studentName}</div><div className="font-mono text-xs text-slate-500">{cert.code} • {new Date(cert.issueDate + 'T00:00:00').toLocaleDateString('pt-BR')}</div></div><button onClick={() => setSelected(cert)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 text-xs font-bold"><Eye className="w-3.5 h-3.5" />Visualizar</button></div>)}</div></section>}
