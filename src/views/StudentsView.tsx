        ...
        <div className="overflow-x-auto"><table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {filteredStudents.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center text-slate-400">Nenhum aluno encontrado.</td></tr> : filteredStudents.map((student) => {
              ...
              return <tr key={student.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40"><td className="px-4 py-3.5"><div className="font-bold text-slate-900 dark:text-white">{student.fullName}</div><div className="text-[11px] text-slate-400">{student.email}</div></td><td className="px-4 py-3.5 font-mono text-slate-600 dark:text-slate-300">{student.documentNumber || '-'}</td><td className="px-4 py-3.5 text-slate-600 dark:text-slate-300">{course?.name || '-'}</td><td className="px-4 py-3.5 text-slate-600 dark:text-slate-300">{student.completionDate ? new Date(`${student.completionDate}T00:00:00`).toLocaleDateString('pt-BR') : '-'}</td><td className="px-4 py-3.5"><span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800"><Award className="w-3 h-3" />{certs.length}</span></td><td className="px-4 py-3.5"><div className="flex justify-end gap-1.5"><button onClick={() => openEmission(student)} className="px-2 py-1 text-[11px] font-semibold text-indigo-700 bg-indigo-50 dark:bg-indigo-950 rounded-lg">Emitir</button><button onClick={() => openEdit(student)} className="p-1.5 text-slate-500 hover:text-indigo-600"><Edit2 className="w-3.5 h-3.5" /></button><button onClick={() => setStudentToDelete(student)} className="p-1.5 text-slate-400 hover:text-rose-600"><Trash2 className="w-3.5 h-3.5" /></button></div></td></tr>;
            })}
          </tbody>
        </table></div>
      </div>
